...
-- =====================================================
-- PASO 2: Crear usuario administrador para Perfumeria Lokeyokiera
-- =====================================================

-- Buscar e insertar el admin automáticamente
WITH tienda_encontrada AS (
  SELECT id, nombre, dominio
  FROM tiendas
  WHERE nombre ILIKE '%lokeyokiera%' OR dominio ILIKE '%lokeyokiera%'
  LIMIT 1
)
...
SELECT
  ...
  'Admin Perfumeria Lokeyokiera',
  true
FROM tienda_encontrada
...
SELECT
  ...
  t.dominio as tienda_dominio,
  au.activo,
  au.creado_en
FROM admin_users au
JOIN tiendas t ON au.id_tienda = t.id
WHERE au.email = 'admin@lokeyokiera.com';
